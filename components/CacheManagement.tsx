...
export default function CacheManagement({ isOpen, onClose }: CacheManagementProps) {
  ...
  const migrateSummaries = async () => {
    setCacheStatsLoading(true)
    try {
      const response = await fetch('/api/migrate-summaries', {
        method: 'POST'
      })
      
      if (!response.ok) {
        throw new Error(`Failed to migrate summaries: ${response.statusText}`)
      }
      
      const data = await response.json()
      console.log('✅ Summaries migrated:', data)
      
      alert(`Migration complete!\nCreated: ${data.results.created} summaries\nSkipped: ${data.results.skipped}\nErrors: ${data.results.errors}`)
      
      // Refresh the stats after migration
      await fetchCacheStats()
    } catch (error: any) {
      console.error('❌ Migrate summaries error:', error)
  ...
  const clearAllCache = async () => {
    if (!confirm('⚠️ This will delete ALL cache data including tickers, backtests, and shares. Are you sure?')) {
      return
    }
    ...
    try {
      ...
      // Refresh the stats after clearing
      await fetchCacheStats()
    } catch (error: any) {
  ...
  const clearBacktests = async () => {
    if (!confirm('⚠️ This will delete ALL backtest results and simulation history. Are you sure?')) {
      return
    }
    ...
    try {
      const response = await fetch('/api/clear-cache', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clearType: 'backtests' })
      })
      
      if (!response.ok) {
        throw new Error(`Failed to clear backtests: ${response.statusText}`)
      }
      
      const data = await response.json()
      console.log('✅ Backtests cleared:', data)
      
      alert(`Cleared ${data.clearedCount} backtest entries successfully!`)
      
      // Refresh the stats after clearing
      await fetchCacheStats()
    } catch (error: any) {
      console.error('❌ Clear backtests error:', error)
      alert(`Clear backtests failed: ${error.message}`)
    } finally {
      setCacheStatsLoading(false)
    }
  }
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex space-x-2">
                ...
                >
                  ...
                  <span>Migrate Summaries</span>
                </button>
                <button
                  onClick={clearBacktests}
                  disabled={cacheStatsLoading}
                  className="flex items-center space-x-1 px-3 py-1 bg-red-50 hover:bg-red-100 text-red-600 rounded text-sm"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                  </svg>
                  <span>Clear Backtests</span>
                </button>
                <button
                  onClick={clearAllCache}
                  disabled={cacheStatsLoading}
                  className="flex items-center space-x-1 px-3 py-1 bg-red-50 hover:bg-red-100 text-red-600 rounded text-sm border border-red-200"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
